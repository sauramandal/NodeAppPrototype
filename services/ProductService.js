...
module.exports = {
  ...
  addProduct: function(product) {
    return new Promise((resolve, reject) => {
      var queryString =
        "INSERT INTO ?? (`product_name`,`product_description`,`product_price`,`product_image`,\
								`created_at`,`company_id`,`category_id`) VALUES (?,?,?,?,?,?,?)";
      var table = ["tb_product"];
      var productObject = [
        product.product_name,
  ...
  getAllCartItems: function(orderId) {
    return new Promise((resolve, reject) => {
      ...
      let queryString =
        "SELECT TP.PRODUCT_NAME, TP.PRODUCT_DESCRIPTION, TP.PRODUCT_PRICE, TOD.QUANTITY, TP.PRODUCT_PRICE, TP.PRODUCT_IMAGE\
  ...
  addOrder: function(userId, orderDate, orderAmount, isPlaced) {
    return new Promise((resolve, reject) => {
      let queryString =
        "INSERT INTO ?? (`USER_ID`,`ORDER_DATE`,`ORDER_AMOUNT`,`ORDER_PLACED`) VALUES (?,?,?,?)";
